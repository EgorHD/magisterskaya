...
class OCRWorker(QObject):
    ...
    def __init__(self, doc: Document, *, lang: str = "ru") -> None:
        super().__init__()
        self.doc = doc
        self.lang = lang
    ...
    def run(self) -> None:
        try:
            engine = get_ocr_engine(lang=self.lang, use_angle_cls=True, use_gpu=False)
            ...
            # Иногда Paddle/PaddleX может "кинуть 0" (или вернуть 0 в неожиданный момент).
            # Чтобы приложение не считало это фатальной ошибкой — трактуем как "текст не распознан".
            if str(e).strip() == "0":
                empty_pages = [OCRPageResult(page_index=p.index, spans=[]) for p in self.doc.pages]
                self.finished.emit(OCRResult(pages=empty_pages))
                return

            # Нормальная диагностика: тип + repr
